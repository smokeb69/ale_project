...
const resolveApiUrl = () => {
  // Use environment variable or default to Forge API
...
export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  ...
  if (!AVAILABLE_MODELS.includes(model)) {
    console.warn(`Model "${model}" not in available models list. Using gpt-4.1-mini as fallback.`);
  }
  ...
  if (model.includes("gemini") || model.includes("gpt-4")) {
    payload.thinking = {
      "budget_tokens": 128
    };
  }
  ...
  console.log(`[LLM] Response received from ${result.model} (${result.usage?.total_tokens || 0} tokens)`);
